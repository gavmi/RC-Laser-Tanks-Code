...
def handle_firing():
    ...
    while blink_count < 10:
        now = time.time()
        if now >= next_toggle:
            emitter.ChangeDutyCycle(100 if emitter_on else 0)
...
def sensor_check_loop():
    ...
    while True:
        if GPIO.input(sensor_vout_pin) == GPIO.LOW and (time.time() - last_hit_time) > 1:
            #print(f"Sensor Vout: {GPIO.input(sensor_vout_pin)}")
            health -= 1
...
try:
    for event in device.read_loop():
        ...
        # Receiver Sensor (Hit)
        # 1.8 - 3.3 reads as high 
        # Anything less reads as low
        # if GPIO.input(sensor_vout_pin) == GPIO.LOW and (time.time() - last_hit_time) > 1:
        #     print(f"Sensor Vout: {GPIO.input(sensor_vout_pin)}")
        #     health = health - 1
        #     last_hit_time = time.time()
...
# To Do --------------------------------------------------------------
# Case for health to decrease motor speed
